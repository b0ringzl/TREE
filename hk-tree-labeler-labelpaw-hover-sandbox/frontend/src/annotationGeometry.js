...
export function shouldRefreshHoverPoint(previousPoint, nextPoint, minDistance = 0.015) {
  if (!previousPoint) return true;
  if (!nextPoint) return false;
  const dx = Number(nextPoint[0]) - Number(previousPoint[0]);
  const dy = Number(nextPoint[1]) - Number(previousPoint[1]);
