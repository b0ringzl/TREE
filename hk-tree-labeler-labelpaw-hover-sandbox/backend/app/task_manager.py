...
from .config import DATASET_DIR, MAX_CONCURRENT_DOWNLOADS, NEAR_REJECT_RADIUS_M, TEMP_DIR, TREE_REGISTRY_DB, assert_inside_root, safe_name
...
from .tree_registry import ensure_tree_registry, get_registry_tree, lookup_tree_id
...
class TaskManager:
    ...
    async def lookup_tree_by_id(self, tree_id: str, registry_id: int | None = None) -> dict:
        await self.client.validate_api_key()
        ensure_tree_registry(TREE_REGISTRY_DB)
        if registry_id is not None:
            entry = get_registry_tree(registry_id, TREE_REGISTRY_DB)
            if not entry:
                raise ValueError(f"Tree registry row not found: {registry_id}")
            matches = [entry]
        else:
            matches = lookup_tree_id(tree_id, TREE_REGISTRY_DB)
        if not matches:
            raise ValueError(f"Tree ID not found in the global tree registry: {tree_id}")
        if len(matches) > 1:
            return {"multiple": True, "matches": [match.to_public_dict() for match in matches]}
        entry = matches[0]
        sample = await self._prepare_sample(entry.species, entry.to_tree_record())
        if not sample:
            raise ValueError(f"No usable Street View image was found near tree ID: {entry.tree_id}")
        self.state.species = entry.species
        return {
            "multiple": False,
            "match": entry.to_public_dict(),
            "sample": {
