...
from .sam_hover_service import hover_worker
from .schemas import ApiKeyRequest, LookupTreeRequest, RejectRequest, SamSegmentRequest, StartTaskRequest, SubmitRequest
from .task_manager import manager
from .tree_registry import ensure_tree_registry, lookup_tree_id, rebuild_tree_registry
...
@app.post("/api/tree-registry/rebuild")
def rebuild_tree_registry_api() -> dict:
    try:
        return rebuild_tree_registry()
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@app.get("/api/tree-registry/lookup")
def lookup_tree_registry_api(tree_id: str) -> dict:
    try:
        ensure_tree_registry()
        matches = lookup_tree_id(tree_id)
        return {"tree_id": tree_id, "matches": [match.to_public_dict() for match in matches]}
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc


@app.post("/api/task/lookup-id")
async def lookup_id_task(payload: LookupTreeRequest) -> dict:
    try:
        return await manager.lookup_tree_by_id(payload.tree_id, payload.registry_id)
    except Exception as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
...
@app.post("/api/task/sam-hover")
def sam_hover_task(payload: SamSegmentRequest) -> dict:
    try:
        hover_worker.cleanup_idle()
        return hover_worker.predict(payload.image, payload.model_key, payload.point)
    except Exception as exc:
        return {"image": payload.image, "candidates": [], "error": str(exc)}
