"""Validate a completed ResNet50 classification run and emit a report."""
...
def load_expected_reviews(config: dict[str, object]) -> dict[str, dict[str, str]]:
    ...
    if any("split" not in item for item in reviews):
        dataset_root = Path(str(config["args"]["dataset_root"]))
        manifest = json.loads(
            (dataset_root / "manifest.json").read_text(encoding="utf-8")
        )
        split_by_key = {
            str(item["sample_key"]): str(item["split"])
            for item in manifest["records"]
        }
    def review_split(item: dict[str, object]) -> str:
        if "split" in item:
            return str(item["split"])
        return split_by_key[str(item["sample_key"])]

    return {
        str(item["sample_key"]): {
            "quality": str(item["quality"]),
            "note": str(item["note"]),
            "split": review_split(item),
        }
        for item in reviews
        if review_split(item) in {"val", "test"}
    }
...
def validate_predictions(
    ...
    for sample_key, review in expected_reviews.items():
        ...
        if row is None:
            raise ValueError(f"Reviewed sample missing from predictions: {sample_key}")
        if row["split"] != review["split"]:
    ...
    if len(marked_rows) != len(expected_reviews):
        raise ValueError(
            f"Quality marker count mismatch: {len(marked_rows)} != {len(expected_reviews)}"
        )
    return {
        "row_count": len(rows),
...
def main() -> None:
    ...
    if missing:
        raise FileNotFoundError(f"Missing B3 artifacts: {missing}")
    ...
    if (
        ...
        raise ValueError(
            ...
            f"{len(class_names)} != {args.expected_class_count}"
        )
    if not str(config["model"]).startswith("torchvision ResNet50"):
        raise ValueError("Run config does not describe ResNet50")
    if len(history) != int(final["epochs_completed"]):
        raise ValueError("History length does not match epochs_completed")
    if [int(item["epoch"]) for item in history] != list(range(1, len(history) + 1)):
    ...
    expected_reviews = load_expected_reviews(config)
